  ...
  useEffect(() => {
    ...
    const updateFavicon = () => {
      setTimeout(() => {
        ...
        let primaryStr = getComputedStyle(root).getPropertyValue('--color-primary').trim();
        
        if (primaryStr && !primaryStr.startsWith('#') && !primaryStr.startsWith('oklch') && !primaryStr.startsWith('rgb') && !primaryStr.startsWith('hsl')) {
