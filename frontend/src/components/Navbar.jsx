    ...
    <nav className="glass-panel rounded-none border-t-0 border-x-0 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
                <Link to={"/settings"} className="btn btn-ghost btn-sm rounded-full gap-2">
                    <Settings className="size-4" />
                    <span className="hidden sm:inline">Settings</span>
                </Link>
                {authUser && <>
                    <Link to={"/profile"} className="btn btn-ghost btn-sm rounded-full gap-2">
                        <User className="size-4" />
                        <span className="hidden sm:inline">Profile</span>
                    </Link>
                    <button onClick={logout} className="btn btn-ghost btn-sm rounded-full gap-2 text-error hover:bg-error/20">
